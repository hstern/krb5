...
import (
	...
	"github.com/hstern/krb5/crypto/rfc8009"
)
...
// TestAes256CtsHmacSha384192_DeriveKey handles test vectors from RFC 8009 Appendix A.
func TestAes256CtsHmacSha384192_DeriveKey(t *testing.T) {
	t.Parallel()

	protocolBaseKey, err := hex.DecodeString("6d404d37faf79f9df0d33568d320669800eb4836472ea8a026d16b7182460c52")
	require.NoError(t, err)
...
func TestAes256CtsHmacSha384192_Crypto(t *testing.T) {
	...
	for _, test := range tests {
		m, err := hex.DecodeString(test.plain)
		require.NoError(t, err)

		b, err := hex.DecodeString(test.encrypted)
		require.NoError(t, err)

		ke, err := hex.DecodeString(test.ke)
		require.NoError(t, err)

		cf, err := hex.DecodeString(test.confounder)
		require.NoError(t, err)

		ct, err := hex.DecodeString(test.cipher)
		require.NoError(t, err)
		...
		assert.Equal(t, test.encrypted, hex.EncodeToString(c))
		...
		assert.Equal(t, test.plain, hex.EncodeToString(p))
		...
		assert.Equal(t, test.hash, hex.EncodeToString(mac))
	}
}

// TestAes256CtsHmacSha384192_VerifyIntegrity handles test vectors from RFC 8009.
func TestAes256CtsHmacSha384192_VerifyIntegrity(t *testing.T) {
	...
	testCases := []struct {
		...
		{"Vector1", "ef5718be86cc84963d8bbb5031e9f5c4ba41f28faf69e73d", "000102030405060708090a0b0c0d0e0f1011121314", "45ee791567eefca37f4ac1e0222de80d43c3bfa06699672a"},
	}
